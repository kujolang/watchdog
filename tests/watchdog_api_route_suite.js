...
async function run() {
	...
	try {
		...
		await assertEndpoint('/api/traces', data => {
			...
			assert.match(String(trace.pricing_source || ''), /^openrouter-public-catalog:2026-08-30/);
		...
		await assertEndpoint('/api/charts/stat-trends', data => {
			...
			assert.ok(Array.isArray(data.canonical_metrics), 'stat-trends canonical_metrics should be an array');
