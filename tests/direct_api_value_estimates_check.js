...
assert(providerCatalog.catalog_id === 'watchdog-provider-catalog:2026-09-20', 'Provider catalog snapshot should be versioned with the refresh date');
...
assert(providerCatalog.models['deepseek-v4-pro'].input_rate_per_million === 1.32, 'DeepSeek V4 Pro cache-miss input pricing should use the current peak rate');
assert(providerCatalog.models['deepseek-v4-pro'].pricing_source === 'deepseek-pricing:2026-09-20:peak-rate', 'DeepSeek V4 Pro provenance should identify the current public pricing tier');
assert(providerCatalog.models['deepseek-chat'].priced_model === 'deepseek-flash', 'Deprecated DeepSeek aliases should resolve to their current documented pricing basis');
assert(providerCatalog.models['deepseek-v4.1-flash'].cached_input_rate_per_million === 0.006, 'DeepSeek V4.1 Flash should preserve current cache-hit pricing');
...
assert(catalog.catalog_id === 'openrouter-public-catalog:2026-09-20', 'Catalog snapshot should be versioned with the refresh date');
