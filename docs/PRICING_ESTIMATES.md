...
The estimate uses standard input and output rates. Cache-read discounts are not applied because the current telemetry contract does not report cached token counts. Qwen's generic aliases assume the 397B Ollama Cloud model. Provider pricing can change, so update the shared pricing table and this document together.
