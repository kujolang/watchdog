    ...
    >
      {Object.entries(config).map(([name, entry]) => {
        ...
        return (
          <li
            key={name}
            className="flex items-center gap-1.5 font-mono text-[11px] text-slate-500 dark:text-slate-400"
          >
            <span
            ...
            {value !== undefined ? (
              <span className="text-slate-950 dark:text-slate-50">{valueFormatter(value)}</span>
            ) : null}
          </li>
        )
      })}
