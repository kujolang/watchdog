...
const VARIANT: Record<TooltipVariant, string> = {
  ...
  "frosted-glass": "bg-white/70 backdrop-blur-sm dark:bg-slate-950/70",
}
    ...
    <AnimatePresence>
      {show && items.length > 0 && (
        <motion.div
          ...
          className={cn(
            "pointer-events-none absolute z-10 rounded-md border border-slate-200 px-2 py-1 shadow-sm dark:border-slate-800",
            VARIANT[variant]
          )}
        >
          {heading && (
            <div className="mb-0.5 font-mono text-[10px] text-slate-500 dark:text-slate-400">
              {heading}
            </div>
          )}
          <div className="flex flex-col gap-0.5">
            {items.map((item) => (
              <div
                key={item.name}
                className="flex items-center gap-1.5 font-mono text-[11px] text-slate-950 tabular-nums dark:text-slate-50"
              ...
              >
                ...
                <span className="ml-auto pl-2 text-slate-950 dark:text-slate-50">
                  {valueFormatter
