...
[![Version](https://img.shields.io/badge/version-1.0.1-black)](https://github.com/kujolang/watchdog)
...
`encoding: "protobuf"` is the recommended OTLP/HTTP transport and sends `application/x-protobuf`; it requires Kujo 1.2.2 or newer for binary request/response bodies. `encoding: "json"` remains the compatibility default when the field is omitted. Set `compression: "gzip"` for bounded binary-safe request compression. Mapping profiles are independently pinned as `otel.base.v1`, `otel.genai.v1`, or `openinference.v1`. Grafana/Tempo, Datadog, Honeycomb, Phoenix, Langfuse, and other compatible destinations should normally receive these profiles through their OTLP endpoint or an OpenTelemetry Collector, not a destination-specific Watchdog trace client.
...
The guarded trace receiver is `POST /telemetry/v2/otlp/v1/traces`. It accepts OTLP/HTTP JSON and Protobuf, optionally gzip-compressed, under the same authentication, request-size, rate, and privacy controls as other ingestion. It retains only spans carrying recognized GenAI/OpenInference/Watchdog semantics, returns OTLP partial success for rejected spans, strips content at the authoritative policy boundary, and rejects generic traces, logs, metrics, malformed wire data, and over-limit decompression. Watchdog is intentionally not a generic OpenTelemetry Collector.
...
- Connected Sources management panel implementation prompt: [docs/plans/connected-sources-management/IMPLEMENTATION_PROMPT.md](docs/plans/connected-sources-management/IMPLEMENTATION_PROMPT.md)
